...
export default async function LanguagePage({ params, searchParams }: { params: Promise<{ lang: string }>; searchParams?: Promise<{ date?: string; mode?: string; month?: string }> }) {
  ...
  return <WisdomExperience focus="library" initialLanguage={requireLanguage(lang)} posts={posts} initialDate={query?.date} initialMode={query?.mode === 'read' || query?.mode === 'watch' || query?.mode === 'listen' || query?.mode === 'pdf' ? query.mode : undefined} initialMonth={query?.month} />;
}
